...
class Camera(object):
	...
	def get_frame(self):
		...
		frame = None
		while not frame:
            read_return_code, frame = self.cam.read()
		return frame
	
	
	def get_fps(self):
		fps = self.cam.get(cv2.CV_CAP_PROP_FPS)
		return fps if fps != -1 else 30.0
	
	
	def get_size(self):
		w = int(self.cam.get(cv2.CV_CAP_PROP_FRAME_WIDTH))
		h = int(self.cam.get(cv2.CV_CAP_PROP_FRAME_HEIGHT))
		return (w,h)
